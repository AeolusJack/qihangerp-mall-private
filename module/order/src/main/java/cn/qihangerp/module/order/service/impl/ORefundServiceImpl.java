...
//        JSONObject refundDetail = jsonObject.getJSONObject("data");
        ...
        String status = refundDetail.getString("status");
        String type = refundDetail.getString("type");
        Integer refundStatus = 1;
        Integer refundType = null;
        Integer hasGoodReturn = 0;
        if(status.equals("USER_CANCELD")||status.equals("RETURN_CLOSED")||status.equals("MERCHANT_REFUND_RETRY_FAIL")||status.equals("MERCHANT_FAIL")){
            //RETURN_CLOSED	退货退款关闭
            //USER_CANCELD	用户取消申请
            //MERCHANT_REFUND_RETRY_FAIL	商家打款失败，客服关闭售后
            //MERCHANT_FAIL	售后关闭
            refundStatus =1;
        } else if (status.equals("MERCHANT_REFUND_SUCCESS")||status.equals("MERCHANT_RETURN_SUCCESS")||status.equals("MERCHANT_EXCHANGE_SUCCESS")) {
        ...
        if (oRefunds == null || oRefunds.isEmpty()) {
            ...
            insert.setOrderNum(refundDetail.getString("orderId"));
            insert.setOrderItemNum(refundDetail.getString("orderId")+refundDetail.getString("skuId"));
            insert.setSkuId(refundDetail.getLong("skuId"));
            insert.setGoodsId(refundDetail.getLong("ogoodsId"));
            insert.setGoodsSkuId(refundDetail.getString("ogoodsSkuId"));
//            insert.setSkuNum(refundDetail.getString("outSkuId"));
...
//            insert.setGoodsSku(refundDetail.getString("orderSkuSpec"));
//            insert.setGoodsImage(refundDetail.getString("orderProductImage"));
            insert.setQuantity(refundDetail.getLong("count"));
