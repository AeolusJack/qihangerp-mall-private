<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="120px">
      ...
      <el-form-item label="电商平台SKUID" prop="wareId">
        <el-input
          v-model="queryParams.wareId"
          placeholder="请输入电商平台SKUID"
          clearable
...
<!--        />-->
<!--      </el-form-item>-->
      <el-form-item label="SKU编码" prop="outerId">
        <el-input
...
<!--      <el-table-column label="平台SKU ID" align="center" prop="skuId" />-->
      <el-table-column label="SKU编码" align="left" prop="outerId" width="250px"/>
      <el-table-column label="商品标题" align="left" prop="skuName" />
      <el-table-column label="规格" align="left" prop="saleAttrs" width="300px">
        <template slot-scope="scope">
          {{ getSkuValues(scope.row.saleAttrs)}}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    getSkuValues(jsonString) {
      // 将字符串转换为 JSON
      const jsonData = JSON.parse(jsonString);

      // 提取 `attrValueAlias` 并组合成一个字符串
      // 提取 `attrValueAlias` 并组合成一个字符串
      // const combinedValue = jsonData
      //   .map(item => item.attrValueAlias && item.attrValueAlias[0])  // 确保 attrValueAlias 不为空
      //   .filter(Boolean)  // 过滤掉 undefined 或 null 的值
      //   .join(' ');  // 用空格连接所有提取的值
      // 提取并解析 `attrValueAlias`，如果它是 JSON 字符串就进行解析
      const combinedValue = jsonData
        .map(item => {
          // 如果 attrValueAlias 是字符串并且是有效的 JSON，则解析它
          let valueAlias = item.attrValueAlias && item.attrValueAlias[0];

          try {
            // 尝试解析 JSON 字符串
            valueAlias = JSON.parse(valueAlias);
          } catch (e) {
            // 如果解析失败，保持原始值
            return valueAlias || '';
          }

          // 如果解析后的值是数组，提取其中的 'value' 字段
          if (Array.isArray(valueAlias)) {
            return valueAlias.map(v => v.value).join(' ');
          }
          return '';
        })
    ...
    /** 查询商品管理列表 */
    getList() {
      if(this.queryParams.wareId && isNaN(this.queryParams.wareId)){
    ...
    handleLink(row) {
      ...
      getGoodsSku(id).then(response => {
        ...
        this.form.skuCode = response.data.outerId
        this.open = true;
      });
    },
    /** 提交按钮 */
