<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="120px">
      ...
      <el-form-item label="电商平台SKUID" prop="skuId">
        <el-input
      ...
      <el-form-item label="商品名称" prop="numIid">
        <el-input
...
<!--      <el-table-column label="平台商品ID" align="center" prop="numIid" />-->
      ...
      <el-table-column label="规格" align="left" prop="propertiesName" >
        <template slot-scope="scope">
          {{ getSkuValues(scope.row.propertiesName)}}
        </template>
      </el-table-column>
    ...
    <el-dialog title="关联ERP商品SKU" :visible.sync="detailOpen" width="560px" append-to-body :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" inline>
        <el-form-item label="SKU编码" prop="skuCode">
          <el-input v-model="form.skuCode" placeholder="请输入SKU编码" style="width:250px" />
...
export default {
  ...
  methods: {
    getSkuValues(input) {
      // 使用正则表达式提取 "颜色分类" 和 "安装方式" 后面的值
      const colorMatch = input.match(/颜色分类:([^;]+)/);
      const installMatch = input.match(/安装方式:([^;]+)/);

      // 提取并拼接结果
      const color = colorMatch ? colorMatch[1] : '';
      const install = installMatch ? installMatch[1] : '';

      // 返回拼接后的字符串
      return `${color} ${install}`.trim();
    },
    /** 查询淘宝订单列表 */
    getList() {
      if(this.queryParams.numIid && isNaN(this.queryParams.numIid)){
