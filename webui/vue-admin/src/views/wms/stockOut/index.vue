<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-select v-model="queryParams.shopId" filterable r placeholder="出库店铺" clearable>
        <el-option
      ...
      <el-form-item label="店铺分组" prop="shopGroupId">
        <el-select v-model="queryParams.shopGroupId" filterable r placeholder="店铺分组" clearable>
          <el-option
...
<!--      <el-table-column label="源单Id" align="center" prop="sourceId" />-->
      ...
      <el-table-column label="出库店铺" align="left" prop="shopId" width="200">
        <template slot-scope="scope">
          <el-tag type="info">{{ shopList.find(x=>x.id === scope.row.shopId) ? shopList.find(x=>x.id === scope.row.shopId).name : '' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="店铺分组" align="left" prop="shopGroupId" >
        <template slot-scope="scope">
          <el-tag type="info">{{ shopGroupList.find(x=>x.id === scope.row.shopGroupId) ? shopGroupList.find(x=>x.id === scope.row.shopGroupId).name : '' }}</el-tag>
        </template>
      </el-table-column>
...
<!--          <el-table-column type="selection" width="50" align="center" />-->
          ...
          <el-table-column label="出库仓位" prop="positionNum"></el-table-column>
...
<!--          </el-table-column>-->
          <el-table-column label="出库数量" prop="outQty" width="110">
            <template slot-scope="scope">
              <el-input v-model.number="scope.row.originalQuantity - scope.row.outQuantity" disabled placeholder="出库数量"  v-if="scope.row.status < 2" />
            </template>
          </el-table-column>
...
export default {
  ...
  created() {
    listGroup().then(response => {
      ...
      listShop({}).then(response => {
        ...
        this.shopList.push({id:0,name:'其他'})
        this.getList();
      });
    });
  ...
  methods: {
    handleAdd(){
      this.$router.push({path:"/wms/stock_out/create"})
    },
    /** 查询出库单列表 */
    getList() {
      this.loading = true;
