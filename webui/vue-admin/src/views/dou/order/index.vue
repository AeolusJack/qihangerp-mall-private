          ...
          <el-table :data="scope.row.items" :show-header="false" :cell-style="{border:0 + 'px' }"  :row-style="{border:0 + 'px' }" >
            ...
            <el-table-column label="商品名" align="left" width="250px" prop="productName" >
              <template slot-scope="scope">
                ...
                <el-tag size="small" v-if="scope.row.mainStatus ===21 || scope.row.mainStatus===22||scope.row.mainStatus ===39">{{scope.row.mainStatusDesc}}</el-tag>
              </template>
            </el-table-column>
<!--            <el-table-column label="SKU名" align="left" prop="spec" width="150"  :show-overflow-tooltip="true"/>-->
            ...
            <el-table-column label="Sku编码" align="left" prop="code" width="200"/>
...
<!--      <el-table-column label="卖家备注" align="center" prop="sellerWords" />-->
      <el-table-column label="收件人信息" align="left" prop="userName" >
        <template slot-scope="scope">
          <span>{{scope.row.maskPostReceiver}}</span>
<!--          <span>{{scope.row.maskPostTel}}</span>  -->
          <br />
          <span> {{scope.row.provinceName}} {{scope.row.cityName}} {{scope.row.townName}}
            </span>
          <p>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="100px" inline>
        <el-descriptions title="订单信息">
          ...
          <el-descriptions-item label="订单状态">
            <el-tag size="small" v-if="form.orderStatus === 1"> 待支付</el-tag>
            <el-tag size="small" v-if="form.orderStatus === 105"> 已支付</el-tag>
            <el-tag size="small" v-if="form.orderStatus === 2"> 备货中</el-tag>
            <el-tag size="small" v-if="form.orderStatus === 101"> 部分发货</el-tag>
            <el-tag size="small" v-if="form.orderStatus === 3"> 已发货</el-tag>
            <el-tag size="small" v-if="form.orderStatus === 4"> 已取消</el-tag>
            <el-tag size="small" v-if="form.orderStatus === 5"> 已完成</el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="状态说明"> {{form.mainStatusDesc}}</el-descriptions-item>
        ...
        <el-descriptions title="付款信息">
          ...
          <el-descriptions-item label="支付金额">{{amountFormatter(null,null,form.payAmount /100,null)}}</el-descriptions-item>

          <el-descriptions-item label="运费">{{amountFormatter(null,null,form.postAmount / 100,null)}}</el-descriptions-item>
          <el-descriptions-item label="订单优惠">{{amountFormatter(null,null,form.promotionAmount / 100,null)}}</el-descriptions-item>
          <el-descriptions-item label="平台优惠">{{amountFormatter(null,null,form.promotionPlatformAmount / 100,null)}}</el-descriptions-item>
          <el-descriptions-item label="店铺优惠">{{amountFormatter(null,null,form.promotionShopAmount / 100,null)}}</el-descriptions-item>
        ...
        <el-descriptions title="收货信息">
          ...
          <el-descriptions-item label="省市区">{{form.provinceName}}{{form.cityName}}{{form.townName}}{{form.streetName}}</el-descriptions-item>
...
<!--        </el-descriptions>-->
        ...
        <el-table :data="goodsList"  style="margin-bottom: 10px;">
          ...
          <el-table-column label="SKU编码" prop="code"></el-table-column>
          <el-table-column label="电商平台SKUID" prop="skuId"></el-table-column>
          <el-table-column label="单价" prop="goodsPrice" :formatter="amountFormatter">
            <template slot-scope="scope">
              {{amountFormatter(null,null,scope.row.goodsPrice / 100) }}
            </template>
          </el-table-column>
          <el-table-column label="数量" prop="itemNum"></el-table-column>
          <el-table-column label="子订单金额" prop="orderAmount">
            <template slot-scope="scope">
              {{amountFormatter(null,null,scope.row.orderAmount / 100) }}
            </template>
          </el-table-column>
        </el-table>
